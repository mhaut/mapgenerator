		...
		if (os.access(sys.argv[1], os.R_OK) is False):
			...
			with open(sys.argv[1]) as csvfile:
				...
				for line in lines:
					...
					content += '\t\t\t<plane id="muro'+str(idObject)+'" nx="'+str(nx)+'" nz="'+str(nz)+'" size="'+str(width)+',3000"  texture="'+texture+'" />\n'
					content += '\t\t</transform>\n'
					idObject += 1
